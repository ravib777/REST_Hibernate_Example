...
@Path("/")
public class EmployeeCertificateResource {
	
	private CertificateService certificateService = new CertificateService();
	
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCertificates(@PathParam(value = "employeeId") int employeeId)
	{
		List<Certificate> certificates = certificateService.getCertificates(employeeId); 
	...
	@Path("/{certificateId}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCertificate(@PathParam(value = "employeeId") int employeeId,@PathParam(value="certificateId") int certificateId )
	{
		Certificate certificate = certificateService.getCertificate(employeeId,certificateId);
	...
	public Response createCertificate(@PathParam(value = "employeeId") int employeeId,Certificate certificate)
	{
		certificateService.createCertificate(employeeId,certificate);
		JSONObject jsonObject = new JSONObject();
		jsonObject= CertificateJSonConverter.convert(certificate);
		return Response.status(200).entity(jsonObject.toString()).build();
	}
